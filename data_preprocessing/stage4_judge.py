...
class MathProblemValidator:
    def __init__(self, model_path: str, tp_size: int, try_num: int, use_fp8):
        ...
        self.sampling_params = SamplingParams(
            temperature=0.8, max_tokens=16384, top_p=0.95  # TODO
        )
...
{aspect}"""
    ...
    def create_prompt(self, source, problem: str) -> List[Dict[str, str]]:
        ...
        if (
            source == "cn_k12"
            or source == "synthetic_amc"
            or source == "amc_aime"
            or source == "synthetic_math"
        ):
...
class DataProcessor:
    ...
    def load_test_data(self):
        dataset = {"synthetic_math": [], "cn_k12": [], "olympiads": []}
...
def main():
    ...
    parser.add_argument(
        "--model_path", type=str, default="gemini-2.0-flash-thinking-exp"
    )  # TODO
    parser.add_argument(
        "--input_file", type=str, default=f"{date}/stage3_final_math_qa.jsonl"
    )
    ...
    args.output_path = os.path.join(
        args.output_path, f"{args.model_path.split('/')[-1]}_{args.turn}.json"
    )
    ...
    for source, data in dataset.items():
        ...
        for i in range(num_questions):
            analysis = validator.analyze_solutions(
                attempts[i * args.try_num : (i + 1) * args.try_num], data[i]["solution"]
            )
            data[i].update({"validation_results": analysis})

        cnt_solu_status = {}
        cnt_answer_match = {}
        for i in range(num_questions):
            solution_majority = data[i]["validation_results"]["solution_status"][
                "majority"
            ]
            cnt_solu_status[solution_majority] = (
                cnt_solu_status.get(solution_majority, 0) + 1
            )

            answer_majority = data[i]["validation_results"]["match_results"]["majority"]
            cnt_answer_match[answer_majority] = (
                cnt_answer_match.get(answer_majority, 0) + 1
            )
